 ...
 * https://leetcode.com/problems/subdomain-visit-count/discuss/
 *
 * algorithms
 * Easy (59.99%)
 * Total Accepted:    2.7K
...
func subdomainVisits(cpdomains []string) []string {
	...
	for _, cpdomain := range cpdomains {
		sigleCpdomain := strings.Split(cpdomain, " ")
		count, _ := strconv.Atoi(sigleCpdomain[0])
		domains := strings.Split(sigleCpdomain[1], ".")
		curDomain := ""
		for i := len(domains) - 1; i >= 0; i-- {
			...
			if preCount, exists := mapRet[curDomain]; exists {
				mapRet[curDomain] = preCount + count
			} else {
				mapRet[curDomain] = count
			}
		}
	}

	for key, value := range mapRet {
		ret = append(ret, strconv.Itoa(value) + " " + key)
	}
